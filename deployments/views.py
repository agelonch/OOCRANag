...
from .orchestration import planification_DL, planification_UL, optim, rb_offer, distance, price, list_bs, mcs
from aloeoCLI.VNFM.deployments.deployments import create, delete
from aloeoCLI.VIM.OpenStack.nova import nova
from aloeoCLI.VIM.OpenStack.ceilometer.ceilometer import statistics
from aloeoCLI.VNFM.VnfmVnf import client
...
def deployment_create(request,id=None):
	...
	if form.is_valid():
		...
		oarea.rb_offer = rb_offer(str(deploy.rb),oarea.rb_offer,int(str(deploy.start).split(':')[0]),int(str(deploy.stop).split(':')[0]),"suma")
		oarea.save()
		deploy.save()
...
def deployment_delete(request,id=None):
	...
	if deploy.name == area.name:
		...
		area.rb_offer = rb_offer(str(deploy.rb),area.rb_offer,int(str(deploy.start).split(':')[0]),int(str(deploy.stop).split(':')[0]),"resta")
		area.save()
...
def autodeploy(request,id=None):
	...
	auto_deploy = Deployment(area=area,name=area.name, start="00:00", stop="23:59",propietario=get_object_or_404(Operator, name = request.user.username))
